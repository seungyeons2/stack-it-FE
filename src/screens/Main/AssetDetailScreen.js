...
const AssetDetailScreen = ({ navigation }) => {
  ...
  const calculatePercentage = (value) => {
    if (!assetData || !assetData.total_asset || assetData.total_asset === 0) return '0%';
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>보유 주식 비율</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.legendContainer}>
          {prepareChartData().map((item, index) => (
            <View key={index} style={styles.legendItemVertical}>
              ...
              <Text style={styles.legendLabel}>{item.name} {calculatePercentage(item.value)}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={styles.stockListContainer}>
          ...
          {assetData.breakdown && assetData.breakdown.length > 0 ? (
            assetData.breakdown.filter(stock => stock.label !== '예수금').map((stock, index) => (
              <View key={index} style={styles.stockItem}>
                <View style={styles.stockInfoTop}>
                  <Text style={styles.stockName}>{stock.label}</Text>
                  <Text style={styles.stockPercentage}>{calculatePercentage(stock.value)}</Text>
                </View>
                <Text style={styles.stockValue}>{formatCurrency(stock.value)}원</Text>
              </View>
            ))
          ) : (
        ...
        <View style={styles.summaryContainer}>
          <View style={styles.summaryItem}>
            ...
            <Text style={styles.summaryValue}>{formatCurrency(assetData.evaluation)}원</Text>
          </View>
          <View style={styles.summaryItem}>
            <Text style={styles.summaryLabel}>예수금</Text>
            <Text style={styles.summaryValue}>{formatCurrency(assetData.cash)}원</Text>
          </View>
          <View style={[styles.summaryItem, styles.totalItem]}>
            <Text style={styles.totalLabel}>총 자산</Text>
            <Text style={styles.totalValue}>{formatCurrency(assetData.total_asset)}원</Text>
          </View>
        </View>
      </ScrollView>
    </View>
